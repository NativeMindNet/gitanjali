...
+ (id)deserialize:(xmlNodePtr)node
{
    ...
    else if([typeStr isEqualToString:@"external-link"])
	{
		typeVal = kControlTypeExternalLink;
	}
    else if([typeStr isEqualToString:@"undo"])
	{
		typeVal = kControlTypeUndo;
	}
	else
	{
		return nil;
	...
	while (dataNode) 
	{
		...
		{
			if(instance.type == kControlTypePageLink || instance.type == kControlTypeExternalLink)
			{			
				NSString* str = [NSString stringWithUTF8String: (const char*)dataNode->children->content];
				instance.argument = str;
			}
			else if(instance.type == kControlTypeToggleAnimations)
			{
    ...
    //the external link and undo can't be checkmark. If so, fix that
    
    if(instance.type == kControlTypeExternalLink || instance.type == kControlTypeUndo)
    {
        instance.checkmark = NO;
    }
