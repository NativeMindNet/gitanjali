...
- (BOOL)_loadPageWithIndex:(int)num animated:(BOOL)animated transition:(UIViewAnimationTransition)transition searchString:(NSString*)string saveNavigationHistory:(BOOL)saveNavigationHistory;
...
- (id)initWithBook:(BBook *)book
{
	if((self = [super init]))
	{
        ...
        _navigationHistory = [[NSMutableArray alloc] initWithCapacity: 0];
	}
...
- (void)viewDidLoad
{
	...
	[self _loadPageWithIndex:pageNum animated:NO transition: UIViewAnimationTransitionCurlDown searchString: nil saveNavigationHistory: NO];
...
{
	[self _updateBookmarks];
	[self _loadPageWithIndex: index animated:NO transition: UIViewAnimationTransitionCurlDown searchString: nil saveNavigationHistory: YES];
...
- (void)_onPrevButton
{	
	[self _loadPageWithIndex:_currentPageIndex - 1 animated:YES transition: UIViewAnimationTransitionCurlDown searchString: nil saveNavigationHistory: YES];
}

- (void)_onNextButton
{	
	[self _loadPageWithIndex:_currentPageIndex + 1 animated:YES transition: UIViewAnimationTransitionCurlUp  searchString: nil saveNavigationHistory: YES];
}
...
- (BOOL)_loadPageWithIndex:(int)num animated:(BOOL)animated transition:(UIViewAnimationTransition)transition searchString:(NSString*)string saveNavigationHistory:(BOOL)saveNavigationHistory
{
    ...
    {
        [_navigationHistory addObject: [NSNumber numberWithInt: _currentPageIndex]];
    }
...
- (void)_finalizePageLoading
{
	...
	if(_animationTransition != UIViewAnimationTransitionNone)
	{
		...
		[UIView setAnimationTransition: _animationTransition forView: _mainView cache: YES];
    ...
    BOOL enableNavHistoryControls = YES;
    
    if(_navigationHistory.count)
	{
		enableNavHistoryControls = YES;
	}
	else
	{
		enableNavHistoryControls = NO;
	}
	
	for(BUIControl* ctrl in _controls)
    {    
        if(ctrl.type == kControlTypeUndo)
        {
            ctrl.enabled = enableNavHistoryControls;
        }
	}
...
- (void)_onChangePageNotification:(NSNotification*)notification
{
	...
	[self _loadPageWithIndex:index animated: NO transition: UIViewAnimationTransitionCurlDown  searchString: highlightedString saveNavigationHistory:YES];
	...
	switch ([controlType intValue]) 
	{
		...
		case kControlTypePageLink:
		{
			...
			[self _loadPageWithIndex: pageIndex 
						...
						searchString: nil
             saveNavigationHistory: YES];
		}
		break;
		
		case kControlTypeToggleAnimations:
		{
			NSArray* animationsThatNeedToBeToggled = info.argument;
			
			for(BAnimation* animation in _pageView.page.animations)
			{
				BOOL needToToggleTheAnimation = NO;
				
				for(NSString* name in animationsThatNeedToBeToggled)
				{
					if([animation.name isEqualToString: name])
					{
						needToToggleTheAnimation = YES;
						break;
					}
				}
				
				if(needToToggleTheAnimation)
				{
        ...
        case kControlTypeExternalLink:
		{
			NSString* link = info.argument;
        ...
        case kControlTypeUndo:
		{
            if(_navigationHistory.count)
            {
                NSInteger prevPage = [[_navigationHistory lastObject] intValue];
                [_navigationHistory removeObject: [_navigationHistory lastObject]];
                
                UIViewAnimationTransition transition;
                
                if(_currentPageIndex < prevPage)
                {
                ...
                [self _loadPageWithIndex:prevPage 
                                animated:YES 
                              transition:transition  
                            searchString: nil
                 saveNavigationHistory: NO];
            }
